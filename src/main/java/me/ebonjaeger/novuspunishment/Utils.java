...
public class Utils {
    ...
    public static Instant addDuration(String arg, Instant from) {
        ...
        switch (unit) {
            case 'S':
                return from.plus(difference, ChronoUnit.SECONDS);
            case 'M':
                return from.plus(difference, ChronoUnit.MINUTES);
            case 'H':
                return from.plus(difference, ChronoUnit.HOURS);
            case 'D':
                return from.plus(difference, ChronoUnit.DAYS);
            default:
                throw new IllegalArgumentException("Unknown unit identifier '" + unit + "'");
        }
    }
